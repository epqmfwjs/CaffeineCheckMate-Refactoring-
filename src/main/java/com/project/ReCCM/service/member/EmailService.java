...
public class EmailService {
    ...
    public boolean sendVerificationEmail(String email) {
        ...
        try {
            ...
            helper.setSubject("CaffeineCheckMate 이메일 인증"); // 이메일 제목

            // HTML 형식의 이메일 내용 작성
            String htmlMsg = "<div style='text-align: center;'>" +
                    "<img src='cid:logoImage' alt='Logo' style='width: 300px; margin-bottom: 20px;'/>" +
                    "<h2>카페인관리 및 정보공유 웹사이트,</h2>" +
                    "<h2>CaffeineCheckMate에 오신 걸 환영합니다.</h2>" +
                    "<p>인증 코드: <strong>" + code + "</strong></p>" +
                    "<br/>" +
                    "<p>CaffeineCheckMate에서 인증 코드를 입력해주세요.</p>" +
                    "</div>";

            helper.setText(htmlMsg, true); // HTML 내용으로 설정

            // 이미지 첨부 (메인 로고)
            helper.addInline("logoImage", new ClassPathResource("static/img/logo.png"));
